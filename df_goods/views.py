...
def index(request):
    ...
    # # 构造上下文
    context = {'title': '首页', 'fruit': fruit,'fish': fish, 'meat': meat, 'egg': egg,'vegetables': vegetables, 'frozen': frozen,
               'fruit2': fruit2, 'fish2': fish2, 'meat2': meat2,'egg2': egg2, 'vegetables2': vegetables2, 'frozen2': frozen2,'guest_cart': 1,'page_name':0,'count':count}
...
def goodlist(request, typeid, pageid, sort):
    ...
    url中的参数依次代表
    typeid:商品类型id;selectid:查询条件id，1为根据id查询，2为根据价格查询，3为根据点击量查询
    """
    count = request.session.get('count')
    # 获取最新发布的商品
    ...
    context = {
        ...
        'typeid': typeid, 'sort': sort,'pindexlist': pindexlist,
        'pageid': int(pageid),'count':count,
    }
...
def detail(request,id):
    ...
    #
    context={
        'title':goods.gtype.ttitle, 'guest_cart':1, 'g':goods, 'newgood':news,
        'id':id, 'isDetail':True, 'list':1, 'goodtype':goodtype, 'count':count,
    }
